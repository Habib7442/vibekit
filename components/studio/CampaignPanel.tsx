...
export function CampaignPanel() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      updateMessage(assistantMsgId, { 
        content: 'Campaign generation failed. Please try again or contact support if the issue persists.', 
        isLoading: false 
      });
    } finally {
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/10 space-y-4">
          <div className="space-y-1.5">
            <label htmlFor="campaign-goal" className="text-[9px] text-indigo-400 font-black uppercase tracking-widest px-1">Campaign Goal</label>
            <select 
              id="campaign-goal"
              value={goal}
          ...
          <div className="space-y-1.5">
            <label htmlFor="campaign-message" className="text-[9px] text-zinc-500 font-black uppercase tracking-widest px-1">Main Message (Optional)</label>
            <textarea 
              id="campaign-message"
              value={message}
          ...
          <div className="space-y-1.5">
            <label htmlFor="campaign-offer" className="text-[9px] text-zinc-500 font-black uppercase tracking-widest px-1">Offer / CTA (Optional)</label>
            <input 
              id="campaign-offer"
              value={offer}
